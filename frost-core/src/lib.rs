...
fn derive_interpolating_value<C: Ciphersuite>(
    ...
    match &signing_package.signing_participants_groups {
        Some(signing_participants_groups) => {
            let signing_participants_group = signing_participants_groups.iter().find(|g| g.contains(signer_id));

            match signing_participants_group {
                Some(group) => compute_lagrange_coefficient(
                    group,
                    None,
                    *signer_id,
                ),
        ...
        None => compute_lagrange_coefficient(
            &signing_package
                .signing_commitments()
                .keys()
                .cloned()
